...
class HuffmanCoding:
    ...
    def __buildCodesHelper(self,root,curr_bits):
        ...
        if root.value is not None:
            ...
            self.__removeCodes[curr_bits] = root.value
            return 

        self.__buildCodesHelper(self,root.left,curr_bits+"0")
        self.__buildCodesHelper(self,root.left,curr_bits+"1")
    ...
    def compress(self):
        ...
        with open(self.path, 'r+') as file , open(output_path, 'wb') as output:
            ...
            # construct thr codes from binary tree
            self.__buildCodes()
    ...
    def decompress(self, input_path):
        ...
        with open(self.path, 'rb') as file , open(output_path, 'w') as output:
            ...
            while byte:
                ...
                bit_string += bits
                byte = file.read(1)
            actual_text = self.__renovePadding(bit_string)
            decompressed_text = self.__decodeText(actual_text)
            output.write(decompressed_text)

path = '/home/user/Documents/sample'
